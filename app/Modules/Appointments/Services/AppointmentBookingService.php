...
class AppointmentBookingService extends BaseService
{
    ...
    /**
     * Receptionist books on behalf of a patient — Cash only.
     *
     * The slot is only checked for availability here, NOT locked.
     * The appointment is created AwaitingPayment with a Pending Cash
     * Payment. Nothing is reserved until confirmCashPayment() succeeds.
     */
     ...
     * for BOTH bookOnBehalf() and createWalkIn() appointments.
     *
     * - If the appointment has a slot (bookOnBehalf flow), the slot is
     *   reserved HERE for the first time, and the appointment becomes
     *   Scheduled.
     * - If the appointment has no slot (walk-in flow), the appointment
     *   becomes CheckedIn directly — the patient is already on site.
     *
     * Rejects (no state change at all) if there is no Pending cash
    ...
    /**
     * Walk-in — Cash only, no Stripe, no deposit. Patient is physically
     * on site but still needs the receptionist to confirm cash receipt
     * via confirmCashPayment() before the appointment is CheckedIn.
     */
